-- ─── BECOMING · Migration 001 ────────────────────────────────────────────────
-- Neues Datenmodell: habits-Tabelle, habit_completions, Soft-Delete, RLS
-- Idempotent: kann mehrfach ausgeführt werden (IF NOT EXISTS, ON CONFLICT, etc.)

BEGIN;

-- ─── 1. Alte habits-Tabelle umbenennen (falls sie NICHT das neue Schema hat) ───
DO $$
BEGIN
  -- Wenn habits-Tabelle altes Schema hat (kein is_default):
...
-- ─── 2. Neue habits-Tabelle (Habit-Definitionen) ──────────────────────────────
...
-- ─── 5. Daten-Migration: alte Completions → neue Tabelle ─────────────────────
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
...
-- ─── 6. deleted_at zu user_personas hinzufügen ───────────────────────────────
...
-- ─── 8. RLS-Policies (idempotent via DROP IF EXISTS) ─────────────────────────

-- habits: Defaults sind öffentlich lesbar; eigene Custom-Habits vollständig
...
-- habit_completions: nur eigene Zeilen
